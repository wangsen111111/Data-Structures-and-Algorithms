...
/*G*/ {14, INF, INF, INF, 8, 9, 0}};
        ...
        System.out.println("没有排序：" + Arrays.toString(edges));
     ...
     * ends[] :数组记录了各个顶点对应的终点是哪个，ends数组是在遍历过程中，逐步形成
     * i:表示传入的顶点对应的下标
     * 返回就是下标为i的这个顶点对应的终点的下标
     */
    ...
    public void kruskal() {
        ...
        int[] ends = new int[edgeNum];//用于保存“已有最小生成树”中的每个顶点在最小生成树的特点
        //创建结果数组，保存最后的生成树
        ...
        //按照边的权值大小进行排序
        sortEdges(edges);
        //遍历edges数组，将边添加到最小生成树中时，判断是准备加入的白能是否形成了贿赂，如果没有，就加入rets，否则不能加入
        for (int i=0;i<edgeNum;i++) {
            //获取到第i条百年上的第一个顶点(起点)
            int p1=getPosition(edges[i].start);
            //获取到第i条边的第2个顶点
            int p2=getPosition(edges[i].end);
            //获取p1这个顶点在已有最小生成树中的节点
            int m=getEnd(ends,p1);
            //获取p2这个顶点在已有最小生成树中的终点
            int n=getEnd(ends,p2);
            //是否构成回路
            if(m!=n){//没有构成回路
                ends[m]=n;//设置m在“已有最小生成树”中的终点<E,F>[0000500000]
